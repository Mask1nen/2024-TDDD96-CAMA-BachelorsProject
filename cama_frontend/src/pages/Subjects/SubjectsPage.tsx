...
import StudyCard from "../Database/StudyCard";  // Adjust the import path as needed
import bild2 from "../../assets/images/bild2.png"; // Placeholder image
import { Study } from "../../api/newTypes";

const categories = ["Mathematics", "Science", "Language"];
...
const SubjectsPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchStudies = async () => {
            if (selectedCategory) {
                setLoading(true);
                console.log(`Fetching studies for category: ${selectedCategory}`);
                const studies = await fetchStudiesByCategory(selectedCategory);
                console.log(`Fetched studies for category ${selectedCategory}:`, studies);
                setStudies(studies);
                setLoading(false);
            }
        };
