...
def test_user_user_factory(cama_user_factory):
    """
    Test creating multiple CamaUser instances using a factory.

    This test creates a batch of CamaUser instances and verifies the correct number 
    of instances are created.
    """
    size = 10
...
def test_study_factory(study_factory):
    """
    Test creating multiple Study instances using a factory.

    This test creates a batch of Study instances and verifies the correct number 
    of instances are created.
    """
    size = 100
...
def test_experiment_factory(experiment_factory):
    """
    Test creating multiple Experiment instances using a factory.

    This test creates a batch of Experiment instances and verifies the correct number 
    of instances are created.
    """
    size = 10
...
def test_effect_data_factory(effect_data_factory):
    """
    Test creating multiple EffectData instances using a factory.

    This test creates a batch of EffectData instances and verifies the correct number 
    of instances are created.
    """
    size = 10
...
def test_database_foriegn_key_whith_cascade(effect_data_factory):
    """
    Test foreign key cascade delete functionality.

    This test creates a batch of EffectData instances, verifies their creation,
    deletes the associated Study instance, and ensures the related EffectData
    and Experiment instances are also deleted.
    """
    size = 10
    effect_data = EffectDataFactory.create_batch(size)
    assert len(effect_data) == size 
    for x in effect_data:
        ...
        assert x.experiment_nr is not None
        study_id = x.experiment_nr.study_id.study_id
        assert x.experiment_nr.study_id is not None
